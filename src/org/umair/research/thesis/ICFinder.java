...
public class ICFinder {
	...
	private String line = "";
	// look up
	private Hashtable<String, Double> lookup = null; // quick look up for synset
														// counts (we require
														// Double as Resnik
														// counts are doubles)
	...
	private double nounsandverbs_sum = 0.0; // ** the ??? normaliser ??? ** for
											// the 'getProbability' method
	...
	private double getProbability(String synset, String pos) {
		...
		if (pos.equalsIgnoreCase("n"))
			probability = (freq / nounroot_sum); // Ted Pedersen et al. use the
													// sum of the noun<root>
													// counts *not* the sum of
													// the noun counts

		if (pos.equalsIgnoreCase("v"))
			probability = (freq / verbroot_sum); // Ted Pedersen et al. use the
													// sum of the verb<root>
													// counts *not* the sum of
													// the verb counts
	...
	// !!! we are using the notion of a 'fake'<root> as per the Perl
	// implementation !!!
	// !!! there is no option to turn the 'fake'<root> off in this
	// implementation - it all gets a bit silly (hard to justify) if we do this
	// !!!
	public double getIC(String synset, String pos) {
		double ic = 0.0;
		// Case 1. There is *no* <lcs>
		// ...............................................................................................................................................
		// If the 'synset' is empty (null Object or an empty String), - this
		// implies that no <lcs>|synset was found for a (pair of synsets) and
		// thus,
		// they must join at an 'imaginary' <root> point in the WordNet space
		// (tree). We call this the'fake'<root>.
		// Further, *if* we are assuming a 'fake' root' (which we do; we default
		// to it as per the Perl implementation), - this implies
		// that it subsumes all other <roots>. This being the case, the
		// 'fake'<root> must then have an Information Content(ic) value of 0
		// as it provides us with zero information
		if (synset == null || synset.length() == 0) {
			return (ic);
		}
		// .......................................................................................................................................................................................
		// Case 2. There is an <lcs> but it has a frequency of zero and thus it
		// has a probability of zero and thus is just not valid as input
		// to the Information Content equation ( we will get 'Infinity') - so,
		// we simply return 0
		...
		// .......................................................................................................................................................................................
		// Case 3. There is an <lcs>, -- it may be a <root> or it may be a
		// boring old synset but - it does have a frequency, thus it does have
		// a probability and thus we may calculate the Information Content for
		// this synset. If the synset is a <root> and there is only 1 such
		// <root> for the POS, then, effectively the Information Contente will
		// be zero, otherwise we should get a value that is greater than zero
		return (ic);
	}

	// utility: get counts for {synsets} | just nouns | just verbs |
	// noun'fake'<root> | verb'fake'<root>
	// these are used to calculate probabilities of {synsets} and to 'back-off'
	// to a <root> value if no LCS exists for 2 words
			...
			while ((line = in.readLine()) != null) {
				...
				for (int i = 0; i < editor.length; i++) {
					...
					if (unit.endsWith("n")) {
						...
						uc = Double.parseDouble(editor[1]); // get the value:
															// the 'count' for
															// the {synset}
	...
	public static void main(String[] args) {
		ICFinder icfinder = new ICFinder(
				"C:/Program Files/WordNet/" + 3.0 + "/WordNet-InfoContent-" + 3.0 + "/ic-semcor.dat");
	}
}
