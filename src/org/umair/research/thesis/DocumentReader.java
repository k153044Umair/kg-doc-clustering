...
public final class DocumentReader
{
   ...
   // private static StanfordLemmatizer _lStanfordLemmatizer = new StanfordLemmatizer();
    ...
    public ArrayList<WordFrequency> getWordFrequencies(List<String> sentences) throws Exception
    {
        ...
        try {
        	...
        	for (String sentence : sentences) {
        		...
        		if (!sentence.isEmpty()) { // Not empty string at this point
    		      ...
    		      //String[] procWords = _lStanfordLemmatizer.lemmatize(sentence).toArray(new String[]{});
        ...
        if(results.size() > 0) {
        	...
        	while (iterator.hasNext()) {
				Map.Entry<java.lang.String, org.umair.research.thesis.WordCounter> entry = (Map.Entry<java.lang.String, org.umair.research.thesis.WordCounter>) iterator
						.next();
...
/* // Processes the given document and returns a word frequency map
	public HashMap<String, WordCounter> getWordFrequenciesFromContent(String content) throws Exception {
		HashMap<String, WordCounter> results = new HashMap<String, WordCounter>();
		try {

			List<String> lemmatizedWords = _lStanfordLemmatizer.lemmatize(content);
			for (String lemmatizedWord : lemmatizedWords) {
				WordCounter count = results.get(lemmatizedWord);
				if (count == null) {
					count = new WordCounter();
					results.put(lemmatizedWord, count);
				}
				count.increment();
			}
		} catch (Exception e) {
