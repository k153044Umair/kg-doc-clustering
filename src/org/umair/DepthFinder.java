...
public class DepthFinder {
	...
	private String vers = "3.0";
	private String wnhome = System.getProperty("WNHOME") + "/dict";
	private String icfile = System.getProperty("WNHOME") + "/WordNet-InfoContent-" + vers + "/ic-semcor.dat";
	...
	private void getHypernyms(HashSet<ISynsetID> synsets, HashSet<ISynsetID> allhypernms) {
		...
		for (ISynsetID s : synsets) {
			...
			hypernyms.addAll(synset.getRelatedSynsets(Pointer.HYPERNYM)); // get
																			...
																			// are
																			// any
			hypernyms.addAll(synset.getRelatedSynsets(Pointer.HYPERNYM_INSTANCE)); // get
																					// the
																					// <hypernyms>
																					// (instances)
																					// if
		...
		while (search) {
			...
			while (!queue.isEmpty()) {
				...
				hyponyms.addAll(synset.getRelatedSynsets(Pointer.HYPONYM)); // get
																			...
																			// are
																			// any
				hyponyms.addAll(synset.getRelatedSynsets(Pointer.HYPONYM_INSTANCE)); // get
																						// the
																						// <hyponyms>
																						// (instances)
																						// if
	...
	public double getSynsetDepth(String word, int senseno, String pos) {
		...
		return ((double) (depths.lastKey() + 2.0)); // ??? node counting, so
													// have to include start and
													// end node
	}
	 ...
	 */
	private void treecreeper(int depth, HashSet<ISynsetID> synsets, TreeMap<Integer, HashSet<ISynsetID>> depths,
